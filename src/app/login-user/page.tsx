...
export default function LoginUserPage() {
  const { login, logout: contextLogout } = useAppContext();
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const [isSocialLoading, setIsSocialLoading] = useState<null | 'google'>(null);

  // Optional: persist login state in React
  useEffect(() => {
    const firebaseUser = auth.currentUser;
    if (firebaseUser?.uid) {
      // Tell Android about the logged-in user
      if (window.AndroidApp) window.AndroidApp.saveUid(firebaseUser.uid);
    }
  }, []);
  ...
  const handleSocialLogin = async (providerName: 'google') => {
    ...
    try {
      ...
      if (firebaseUser?.email && firebaseUser?.uid) {
        ...
        // Tell Android about logged-in UID
        if (window.AndroidApp) window.AndroidApp.saveUid(firebaseUser.uid);
      }
  ...
  const handleLogout = async () => {
    ...
    // Clear React state / context
    contextLogout?.();

    // Clear Android UID
    if (window.AndroidApp) window.AndroidApp.clearUid();
