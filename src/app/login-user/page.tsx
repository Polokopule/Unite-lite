...
export default function LoginUserPage() {
  ...
  const handleSocialLogin = async (providerName: 'google' | 'facebook') => {
    ...
    try {
      ...
      // The user object will be created/retrieved in the context's login function
      // which is now implicitly called by the auth state change.
      // We still need to call our app's login to create the user profile in RTDB.
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // onAuthStateChanged in context handles user state
      // We still need to call our app's login to create the user profile in RTDB.
      ...
      toast({
        variant: "destructive",
        title: "Login Failed",
        description: "Invalid credentials or user does not exist. Please check your email and password or sign up.",
      });
    } finally {
      setIsLoading(false);
    }
